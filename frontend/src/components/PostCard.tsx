...
export default function PostCard({ post, onLike, onDelete, onComment }: PostCardProps) {
  ...
  return (
    <article className={`mx-3 my-2.5 rounded-3xl overflow-hidden bg-card shadow-card border border-border/60 transition-shadow hover:shadow-card-hover ${post.imageUrl ? '' : 'border-l-4 border-l-primary/70'}`}>
      ...
      <div className="px-4 py-2.5 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <span className="font-semibold text-xs text-foreground truncate block leading-tight">{post.authorName}</span>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          {post.isOwn && (
            <>
              <button className="flex items-center justify-center w-7 h-7 rounded-full bg-muted text-muted-foreground hover:bg-primary/10 hover:text-primary transition-all duration-200">
