...
import { SAMPLE_NOTIFICATIONS, type Notification, formatRelativeTime } from '../lib/mockData';
...
const NotificationIcon = ({ type }: { type: Notification['type'] }) => {
  if (type === 'like') return <Heart className="w-3.5 h-3.5 text-red-500 fill-current" />;
  if (type === 'follow') return <UserPlus className="w-3.5 h-3.5 text-primary" />;
  return <MessageCircle className="w-3.5 h-3.5 text-blue-500" />;
};

export default function NotificationsPage({ navigate: _navigate }: NotificationsPageProps) {
    ...
    <div className="flex flex-col">
      {unreadCount > 0 && (
        <div className="px-4 py-2 bg-primary/5 border-b border-border">
          <p className="text-xs text-primary font-medium">{unreadCount} new notification{unreadCount > 1 ? 's' : ''}</p>
        </div>
      )}
        ...
        {notifications.map(notification => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">{formatRelativeTime(notification.timestamp)}</p>
            </div>
