      ...
      <div className="px-4 py-2">
        ...
        {!isLoading && results && results.length > 0 && (
          <div className="space-y-1 py-2">
            {results.map((user) => {
              const avatarSrc = user.avatar || generateAvatarUrl(user.displayName);
              const isFollowing = following.has(user.name);
              return (
                <div key={user.name} className="flex items-center gap-3 py-2.5 px-1 rounded-xl hover:bg-muted/50 transition-colors">
