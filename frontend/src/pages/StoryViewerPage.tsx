...
export default function StoryViewerPage({ navigate, storyId }: StoryViewerPageProps) {
    ...
    <div className="fixed inset-0 bg-black z-50 flex flex-col max-w-md mx-auto">
      ...
      <div className="absolute top-0 left-0 right-0 z-10 px-2 pt-2">
        <div className="h-0.5 bg-white/30 rounded-full overflow-hidden">
          <div className="h-full bg-white rounded-full animate-[progress_5s_linear_forwards]" style={{ width: '100%', animation: 'none', transform: 'scaleX(0)', transformOrigin: 'left', transition: 'transform 5s linear' }} />
        </div>
      </div>
      ...
      <div className="absolute top-4 left-0 right-0 z-10 flex items-center justify-between px-4 pt-4">
        <div className="flex items-center gap-2">
          <img src={story.authorAvatar} alt={story.authorName} className="w-8 h-8 rounded-full border-2 border-white object-cover" />
          <div>
            <p className="text-white font-semibold text-sm">{story.authorName}</p>
            <p className="text-white/70 text-xs">{formatRelativeTime(story.timestamp)}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {story.isOwn && (
          ...
          <button onClick={() => navigate('home')} className="p-2 rounded-full bg-black/30">
