...
import { SAMPLE_CONVERSATIONS, SAMPLE_MESSAGES, type Message, formatRelativeTime, generateAvatarUrl } from '../lib/mockData';
...
export default function ChatThreadPage({ conversationId }: ChatThreadPageProps) {
  const conversation = SAMPLE_CONVERSATIONS.find(c => c.id === conversationId) || SAMPLE_CONVERSATIONS[0];
  const [messages, setMessages] = useState<Message[]>(SAMPLE_MESSAGES[conversationId] || SAMPLE_MESSAGES['1']);
  const [newMessage, setNewMessage] = useState('');
  const messagesEndRef = useRef<HTMLDivElement>(null);
  const { data: userProfile } = useGetCallerUserProfile();

  const myAvatar = userProfile?.avatar || generateAvatarUrl(userProfile?.displayName || 'You');
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-7rem)]">
      ...
      <div className="px-4 py-2 border-b border-border flex items-center gap-3 bg-background">
        <img src={conversation.participantAvatar} alt={conversation.participantName} className="w-8 h-8 rounded-full object-cover" />
        <div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
        {messages.map(msg => (
          <div key={msg.id} className={`flex items-end gap-2 ${msg.isOwn ? 'flex-row-reverse' : 'flex-row'}`}>
            ...
            <div className={`max-w-[70%] ${msg.isOwn ? 'items-end' : 'items-start'} flex flex-col gap-0.5`}>
              <div
              ...
              <p className="text-[10px] text-muted-foreground px-1">{formatRelativeTime(msg.timestamp)}</p>
            </div>
          </div>
        ))}
