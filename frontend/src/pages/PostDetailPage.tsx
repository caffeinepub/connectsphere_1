...
import { SAMPLE_POSTS, type Comment, formatRelativeTime, generateAvatarUrl } from '../lib/mockData';
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      <div className="px-4 py-4 border-b border-border">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">@{post.authorUsername} · {formatRelativeTime(post.timestamp)}</p>
          </div>
        </div>
        ...
        >
          <Heart className={`${liked ? 'fill-current' : ''}`} style={{ width: '18px', height: '18px' }} />
            ...
            <div className="flex-1 bg-muted rounded-2xl px-3 py-2">
              <div className="flex items-center justify-between mb-0.5">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-muted-foreground">{formatRelativeTime(comment.timestamp)}</span>
                  {comment.isOwn && (
      ...
      <div className="border-t border-border px-4 py-3 flex items-center gap-3 bg-background">
        <img src={myAvatar} alt="You" className="w-8 h-8 rounded-full object-cover shrink-0" />
